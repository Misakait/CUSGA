...
@export var color_active_font: Color = Color(1.0, 1.0, 0.0)
## 行动块的半透明背景底色
...
## 存储当前在轴上实际展示的 UI 节点池，索引对应顺序位置
var ui_slots: Array[Label] = []
## 内部记录的当前正在行动的实体对象
...
func _refresh_ui() -> void:
	var target_count = _predicted_sequence.size()
	
	# 如果 UI 槽位多了，就隐去或释放多余的节点
	while ui_slots.size() > target_count:
		var slot = ui_slots.pop_back()
		slot.queue_free()
		
	# 如果 UI 槽位不够，则动态创建新的补充进来
	while ui_slots.size() < target_count:
		var label = Label.new()
		label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
		# 将缩放中心设置在块的中心点，防止缩放时发生偏移
		label.pivot_offset = Vector2(ui_item_size.x / 2.0, ui_item_size.y / 2.0)
		label.custom_minimum_size = ui_item_size
		...
		add_child(label)
		ui_slots.append(label)

	# 遍历每个位置进行渲染
	for i in range(target_count):
		var entity = _predicted_sequence[i]
		var label = ui_slots[i]
		
		# 绑定当前槽位对应的实体数据，以便后续处理高亮互动时能认出谁是谁
		label.set_meta("entity", entity)
		
		# 更新实体名称的文本内容
		label.text = _get_name(entity)

		# 根据实体类型设置颜色区分
		if "player" in entity.name.to_lower():
			label.modulate = color_player
		else:
			label.modulate = color_enemy

		# 第一个索引（顶端）通常为当前行动实体，采用最大号缩放且文字高亮
		if i == 0 and entity == _current_active_entity:
			_tween_scale(label, scale_current)
			label.add_theme_color_override("font_color", color_active_font)
		else:
