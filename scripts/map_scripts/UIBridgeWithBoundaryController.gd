...
## 管理无连接方向的桥口封闭；只操作本容器的四个直接瓦片子节点。
const LAYER_NAMES: Array[StringName] = [
	&"UpBridgeWithBoundary", &"RightBridgeWithBoundary", &"DownBridgeWithBoundary", &"LeftBridgeWithBoundary",
]
...
func configure_room_context(context: RoomContext) -> bool:
	if not validate_room_context(context):
		push_error("UIBridgeWithBoundaryController 收到无效上下文或缺少有效方向瓦片。")
		return false
	for direction in range(LAYER_NAMES.size()):
		var active: bool = not context.has_connection(direction)
