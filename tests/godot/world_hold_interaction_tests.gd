...
const WORLD_HOLD_CONTROLLER_SCRIPT := "res://core/gameflow/world_hold_interaction_controller.gd"
const WORLD_HOLD_INDICATOR_SCRIPT := "res://core/ui/hud/world_hold_progress_indicator.gd"
...
func _run() -> void:
	_test_hold_duration_conversion()
...
func _test_hold_duration_conversion() -> void:
	# 迁移后的 GDScript 常量类必须保留原有零、正数和负数边界行为。
	_assert(
...
func _test_controller_has_default_indicator_path() -> void:
	...
	# 通过动态属性读取验证 GDScript 实例的真实默认值，而不是只检查场景文本。
	var progress_indicator_path: NodePath = controller.get("progress_indicator_path")
...
func _test_indicator_visibility_and_progress_reset() -> void:
	...
	_assert(not bool(indicator.get("is_hold_progress_visible")), "鼠标圆环默认应隐藏。")
	_assert(is_equal_approx(float(indicator.get("ring_radius")), 16.0), "鼠标圆环默认半径应为 16 像素。")
	_assert(is_equal_approx(float(indicator.get("ring_width")), 6.0), "鼠标圆环默认线宽应为 6 像素。")
	_assert(Vector2(indicator.get("cursor_offset")).is_equal_approx(Vector2.ZERO), "圆环默认偏移应为零，确保圆心落在目标右下角。")
	...
	await process_frame
	indicator.call("set_hold_progress_target", target)
	# 读取公开锚点坐标，避免测试依赖不可见的 DrawArc 像素输出。
	var target_anchor: Vector2 = indicator.call("get_current_hold_target_screen_position")
	_assert(target_anchor.is_equal_approx(Vector2(180.0, 90.0)), "圆环应锚定到交互目标的右下角。")
	indicator.call("set_hold_progress", 0.5)
	_assert(bool(indicator.get("is_hold_progress_visible")), "开始长按后应显示鼠标圆环。")
	_assert(is_equal_approx(float(indicator.get("current_progress")), 0.5), "圆环应保存传入的归一化进度。")

	indicator.call("clear_hold_progress")
	_assert(not bool(indicator.get("is_hold_progress_visible")), "取消或完成后圆环应立即隐藏。")
	_assert(is_equal_approx(float(indicator.get("current_progress")), 0.0), "清理圆环时应将进度复位为零。")
...
func _test_cancelled_hold_does_not_complete() -> void:
	...
	controller.set("progress_indicator_path", NodePath("Indicator"))
	...
	controller.call("begin_hold", owner, 10, Callable(tracker, "mark_completed"), owner)
	await process_frame
	controller.call("cancel_hold_for", owner)
	await create_timer(1.1).timeout

	_assert(tracker.call_count == 0, "提前松开长按不得执行局外交互回调。")
	_assert(not bool(indicator.get("is_hold_progress_visible")), "提前松开后不得残留鼠标圆环。")
