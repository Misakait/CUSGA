...
`LisBam` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/LisBam/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/LisBam/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee LisBam` to
...
python ./.trellis/scripts/task.py archive 00-join-lisbam
```
